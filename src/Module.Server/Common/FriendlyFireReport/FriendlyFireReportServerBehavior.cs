...
internal class FriendlyFireReportServerBehavior : MissionNetwork
{
    ...
    private int _friendlyFireReportDecaySeconds;
    ...
    private void UpdateFromServerConfig()
    {
        ...
        Debug.Print($"[FF Server] UpdateFromServerConfig-- MaxHits: {_friendlyFireReportMaxHits} DecaySeconds: {_friendlyFireReportDecaySeconds} ReportEnabled: {_isFriendlyFireReportEnabled}", 0, Debug.DebugColor.Cyan);
    ...
    private void OnFriendlyFireReportRecieved(NetworkCommunicator peer, FriendlyFireReportClientMessage message)
    {
        ...
        Debug.Print($"[FF Server]  Victim: {peer.UserName} Reported: {attackingPeer.UserName} Dmg: {recentHit.Damage} Hits: {countActive}/{_friendlyFireReportMaxHits}", 0, Debug.DebugColor.Red);

        // Notify the attacker about the report
        SendClientDisplayMessage(attackingPeer, $"[FF] {peer.UserName} reported your team hit (Dmg: {recentHit.Damage}). {countActive}/{_friendlyFireReportMaxHits} team hits until getting kicked.", FriendlyFireMessageMode.TeamDamageReportForAttacker);
        ...
        {
            SendClientDisplayMessageToAdmins($"[FF] {attackingPeer.UserName} reported by {peer.UserName}. [{countActive}/{_friendlyFireReportMaxHits}] decayed: {countDecayed} not reported: {countNotReported} on reporter: {countAttacksOnVictim}");
        }
        ...
        if (countActive >= _friendlyFireReportMaxHits)
        {
            ...
            SendClientDisplayMessage(attackingPeer, $"[FF] You have been kicked for excessive team hits ({_friendlyFireReportMaxHits})", FriendlyFireMessageMode.TeamDamageReportKick);
            SendClientDisplayMessageToAllExcept(attackingPeer, $"[FF] {attackingPeer.UserName} has been kicked for excessive team hits ({_friendlyFireReportMaxHits}).", FriendlyFireMessageMode.TeamDamageReportKick);
